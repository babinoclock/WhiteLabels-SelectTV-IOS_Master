...
//    [_foregroundScrollView setFrame:CGRectMake(0, 0, _foregroundScrollView.frame.size.width, _foregroundScrollView.frame.size.height/2)];
...
-(IBAction)didClickOkButtonAction:(id)sender
{
    ...
    if(lastindex==0){
        [self didscrollIndex:1];
    }
    if(lastindex==1){
        [self didscrollIndex:2];
    }
    ...
    if(lastindex==3){
        [self didscrollIndex:4];
    }
    if(lastindex==4){
        [self didscrollIndex:5];
    }
