...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //cell.blockTitle.text = [cellData objectForKey:@"name"];
    NSString* strPosterUrl = cellData[@"image"];
    ...
    [cell.blockImage setImageWithURL:imageUrl];
...
-(IBAction)actionRight:(UIButton *)sender {
    ...
    if (count<=self.collectionData.count-1) {
       // [self snapRightToCellAtIndex:count section:(int)currentItem.section withAnimation:YES];//paas index here to move to.
        [self.collectionView setContentOffset:CGPointMake(500, 0) animated:YES];
    }
