...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    CGRect containerRect = self.dialogView.frame;
    
    if (!CGRectContainsPoint(containerRect, location)) {
        if ([self.delegate respondsToSelector:@selector(customIOS7dialogDismiss)]) {
            [self.delegate customIOS7dialogDismiss];
        }
    }
}
